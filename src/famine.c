...
int do_the_job(char buff[], size_t size, char *path, unsigned int *n_loaded) {
	...
	if (size < sizeof(Elf64_Ehdr) || buff[EI_MAG0] != 0x7f || buff[EI_MAG1] != 'E' || buff[EI_MAG2] != 'L' ||
	    buff[EI_MAG3] != 'F' || buff[EI_CLASS] != ELFCLASS64) {
		return (1);
	}
	Elf64_Ehdr *hdr;
	Elf64_Phdr *phdr;
...
void
new_file(char buf[], size_t size, size_t end_of_text, const char *path, Elf64_Addr old_e_entry, unsigned int n_loaded,
         size_t padding) {
	int fd;
	char tmp[125];
